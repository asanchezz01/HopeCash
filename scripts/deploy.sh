...
# Migração única de instalações antigas: importa o TXT legado e, ao regravar o
# .env abaixo, persiste somente CEREBRAS_API_KEY em texto direto.
if [ -z "$CEREBRAS_API_KEY" ] \
  && [ -n "${CEREBRAS_API_KEY_FILE:-}" ] \
  && [ -r "$CEREBRAS_API_KEY_FILE" ]; then
  CEREBRAS_API_KEY="$(tr -d '\r\n' < "$CEREBRAS_API_KEY_FILE")"
fi
unset CEREBRAS_API_KEY_FILE
CEREBRAS_BASE_URL="${CEREBRAS_BASE_URL:-https://api.cerebras.ai/v1}"
CEREBRAS_TIMEOUT_MS="${CEREBRAS_TIMEOUT_MS:-30000}"
CEREBRAS_REASONING_EFFORT="${CEREBRAS_REASONING_EFFORT:-low}"
CEREBRAS_MODEL="${CEREBRAS_MODEL:-gpt-oss-120b}"
CEREBRAS_MODEL_CHAT="${CEREBRAS_MODEL_CHAT:-$CEREBRAS_MODEL}"
CEREBRAS_MODEL_FAST="${CEREBRAS_MODEL_FAST:-$CEREBRAS_MODEL}"
...
LLM_RETRY_MAX_WAIT_MS="${LLM_RETRY_MAX_WAIT_MS:-15000}"
...
CEREBRAS_TIMEOUT_MS=$CEREBRAS_TIMEOUT_MS
CEREBRAS_REASONING_EFFORT=$CEREBRAS_REASONING_EFFORT
CEREBRAS_MODEL=$CEREBRAS_MODEL
CEREBRAS_MODEL_CHAT=$CEREBRAS_MODEL_CHAT
CEREBRAS_MODEL_FAST=$CEREBRAS_MODEL_FAST
GROQ_API_KEY=$GROQ_API_KEY
GROQ_API_KEY_FILE=$GROQ_API_KEY_FILE
...
LLM_RETRY_MAX_WAIT_MS=$LLM_RETRY_MAX_WAIT_MS
